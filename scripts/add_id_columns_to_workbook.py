#!/usr/bin/env python3
"""
Add ID columns as the first column to all sheets in the Revenue Cloud workbook
"""
import pandas as pd
import os
...
def main():
    ...
    output_file = os.path.join(data_dir, 'Revenue_Cloud_Complete_Upload_Template_FINAL_with_IDs.xlsx')
    
    if not os.path.exists(input_file):
        print(f"Error: Input file not found: {input_file}")
        return
    
    add_id_columns_to_workbook(input_file, output_file)
    
    # Option to replace the original file
    response = input("\nDo you want to replace the original file? (yes/no): ").lower()
    if response == 'yes':
        import shutil
        ...
        print(f"Replacing original file: {input_file}")
        shutil.move(output_file, input_file)
        print("Original file replaced successfully!")
