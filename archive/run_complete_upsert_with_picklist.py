...
including the new AttributePicklist sheet.
"""
...
class CompleteUpsertRunner:
    def __init__(self):
        ...
        self.sheet_configs = [
            ...
            {'sheet': '14_AttributePicklist', 'object': 'AttributePicklist', 'external_id': 'External_ID__c'},  # NEW
    ...
    def excel_to_csv(self, sheet_name):
        ...
        try:
            ...
            if 'Name' in df.columns:
                df = df[df['Name'].notna()]
            elif 'Id' in df.columns and sheet_name != '14_AttributePicklist':  # AttributePicklist may have empty Ids
                df = df[df['Id'].notna()]
            elif 'External_ID__c' in df.columns and sheet_name == '14_AttributePicklist':
                df = df[df['External_ID__c'].notna()]
            ...
            # For AttributePicklist, ensure we have External_ID__c
            if sheet_name == '14_AttributePicklist' and 'External_ID__c' not in df.columns:
                df['External_ID__c'] = df.apply(lambda row: row.get('Code', f"APL_{row.name}"), axis=1)
    ...
    def run_complete_upsert(self):
        ...
        print("=" * 70)
        print("COMPLETE REVENUE CLOUD UPSERT PROCESS (WITH ATTRIBUTEPICKLIST)")
        print("=" * 70)
        ...
        pass2_start = 9  # Index where pass 2 starts (after base objects)
